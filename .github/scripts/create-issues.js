...
process.on('SIGINT', () => { closeLog(); process.exit(1); });
...
const ghRetry = async (argsOpts, max = 5) => {
  for (let attempt = 1; attempt <= max; attempt++) {
    try {
      ...
      if (/rate limit|secondary rate|abuse|502|503|timeout|ECONNRESET/i.test(out)) {
      ...
      if (childNum) {
        try {
          await ghRetry([
            ...
            parentNum.toString(),
            '--repo', REPO,
            '--body', `Linked child task: #${childNum}`
          ]);
...
(async () => {
  ...
  let childrenCreated = 0, childrenSkipped = 0;
  let standaloneCreated = 0, standaloneSkipped = 0;
...
`);
    ...
    if (errors === 0 && !DRY_RUN && fs.existsSync(STATE_FILE)) {
      ...
      log(`\n⚠️ Finished with ${errors} error(s). State file preserved so you can safely retry.`);
    }

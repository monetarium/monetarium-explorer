...
describe('ticketpoolPrice.toColumns', () => {
  it('returns [prices, mem, imm, live, pts] with no mempool when settings is empty', () => {
    ...
    expect(cols[4]).toEqual([null, null, null]) // pts all null
  })
...
describe('ticketpoolPrice overlay points series', () => {
  const overlay = ticketpoolPrice.series[3]
...
describe('ticketpoolPrice.columns', () => {
  it('returns 5 columns (x, mem, imm, live, overlay)', () => {
    const cols = ticketpoolPrice.toColumns(base, {})

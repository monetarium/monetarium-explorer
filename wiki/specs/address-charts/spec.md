...
- Если в `ActiveCoins` ровно одна монета — селектор **задизейблен** (`disabled`) и обёрнут в `<span title="This address only has {coinSymbol} transactions">`, чтобы tooltip работал на disabled-элементе. Скрывать селектор полностью не следует: пользователь должен видеть активную монету и понимать почему выбор недоступен.
...
- Селектор монеты строится по `ActiveCoins`; если у адреса одна монета — селектор задизейблен с tooltip (не скрыт).

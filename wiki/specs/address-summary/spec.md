...
- SKA-строка для монеты из `ActiveCoins` показывается **всегда** (даже при нулевом балансе, если у адреса есть история для этой монеты) — аналогично VAR. `ActiveCoins` строится по факту наличия строк в `addresses`, а не по текущему балансу (см. address-overview §3).

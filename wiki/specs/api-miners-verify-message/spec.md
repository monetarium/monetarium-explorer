...
- **VAR и SKA имеют разную точность и представление** ([core/constraints.md](../../core/constraints.md) C1, [CLAUDE.md](../../../CLAUDE.md)): VAR — 8 знаков, помещается в `float64`/`INT8`; SKA — 18 знаков, атомы только как `big.Int`-строки. **Для счётчика майнеров это не важно** — он оперирует количеством, а не суммами, и равен для всех монет: майнится только VAR, но адрес майнера может быть любым (§1 критерий 3).
...
| Ограничение частоты запросов к `/verify-message`? | **Нет, неограниченно.** Операция CPU-лёгкая (одна проверка ECDSA), аутентификации нет, риска спама по стоимости нет |
...
3. Счётчик не зависит от типа монеты: учитываются майнеры всех монет, добывшие блок в окне (§0).
...
- Не добавляется лимит запросов к `/verify-message` (§0.1).

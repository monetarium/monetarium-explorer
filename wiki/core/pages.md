...
| `/hashrate-shares`             | `HashrateShares` (in `internal/explorer/hashrate_shares.go`)       | Per-miner share of network hashrate, approximated from PoW reward (coinbase) outputs per reward address: SVG pie + ranked table + CSV export. Page shell only; data comes from `/hashrate-shares/data` (JSON, uncached). See [specs/hashrate-shares-block-range/spec.md](../specs/hashrate-shares-block-range/spec.md). |
...
| `/agendas`                     | `AgendasPage`                                                      | Consensus-deployment vote agendas: list with live RCI/SVI progress and quorum/approval meters. Coin-agnostic (vote counts only). On simnet (`voteTracker == nil`) shows an "agendas disabled on simnet" status page. Enabled in PR #395. |
| `/agenda/{agendaid}`           | `AgendaPage` (with `AgendaPathCtx`)                                | Single-agenda detail: choice tally table, timeline, two Dygraphs (cumulative vote choices + by-block). Joins BoltDB metadata with Postgres `agenda_votes` tallies. Enabled in PR #395. |
...
- Features that don't exist on Monetarium (treasury, proposals) are not surfaced via UI links. The same applies to `/market` — until any Monetarium coin is listed on an exchange, the market page and `exchanges` pipeline have no data to show. (Agendas — consensus-deployment voting — *does* exist on Monetarium and is an active page again as of PR #395.)

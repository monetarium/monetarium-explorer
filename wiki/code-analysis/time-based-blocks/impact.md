...
**Trigger:**
Editing the `DATE_TRUNC` expression in `SelectBlocksTimeListingByLimit` ([db/dcrpg/internal/blockstmts.go:151-165](../../../db/dcrpg/internal/blockstmts.go)) and changing the `time at time zone 'utc'` cast in only one of the two places it appears (the `SELECT … AS index_value` projection at line 151 vs the `GROUP BY` at line 163), or dropping the UTC cast to "use local time".
...
**Description:**
The scan is positional across 11 destinations (`indexVal, endBlock, txs, tickets, votes, revocations, blockSizes, blocksCount, startTime, endTime, coinTxStats`). A count/type mismatch makes `rows.Scan` error and the handler renders `"The specified block intervals could be not found"` ([cmd/dcrdata/internal/explorer/explorerroutes.go:584-589](../../../cmd/dcrdata/internal/explorer/explorerroutes.go)). The dangerous case is reordering two `uint64` columns (e.g. `votes`/`revocations`): no error, the wrong totals show in `timelisting.tmpl` columns. Note `txs` is additionally post-mutated by `parseCoinTxStats(coinTxStats, txs)` ([queries.go:947](../../../db/dcrpg/queries.go)) and feeds `TxCount = txs + tickets + revocations + votes` ([queries.go:955](../../../db/dcrpg/queries.go)) — a `coin_tx_stats` shape change there silently corrupts both `Transactions` and the derived `TxCount`.

...
([explorerroutes.go:2040](../../../cmd/dcrdata/internal/explorer/explorerroutes.go#L2040))
...
- A new row that *is* a raw `chaincfg.Params` field needs no handler change —
  ...
  embedding.
- `ExtendedParams` is an anonymous struct redeclared inside the handler body (`explorerroutes.go:2075-2080`);

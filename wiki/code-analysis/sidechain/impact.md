...
**Trigger:** any change to [db/dbtypes/types.go:2237-2244](../../../db/dbtypes/types.go#L2237-L2244) — field add, remove, reorder, or type change.
...
|---|---|---|---|
| `retrieveSideChainBlocks` ([queries.go:4007-4017](../../../db/dcrpg/queries.go#L4007-L4017)) | `SelectSideChainBlocks` | `is_valid, height, previous_hash, hash, next_hash` | `is_mainchain` (filtered by WHERE) |
| `retrieveDisapprovedBlocks` ([queries.go:4027-…](../../../db/dcrpg/queries.go#L4027)) | `SelectDisapprovedBlocks` | `is_mainchain, height, previous_hash, hash, next_hash` | `is_valid` (filtered by WHERE) |
...
**Trigger:** edit either `SelectSideChainBlocks` ([blockstmts.go:180-184](../../../db/dcrpg/internal/blockstmts.go#L180-L184)) or the matching `rows.Scan` ([queries.go:4125](../../../db/dcrpg/queries.go#L4125)) without updating the other.
...
**Why:** the read query (`retrieveSideChainBlocks`) and the writer (`setMainchainByBlockHash` per row, plus per-row updates to transactions / votes / tickets / addresses in [queries.go:4545-…](../../../db/dcrpg/queries.go#L4545)) share no application-level lock. Postgres MVCC isolates the SELECT, but the writer commits row-by-row, so the next request can see a different set.

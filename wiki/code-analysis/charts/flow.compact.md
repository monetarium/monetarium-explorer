...
- **TurboQuery + `Zoom.validate` / `Zoom.project`** drive all URL state, same as the address controller, but charts additionally projects zoom across data-range changes (line 901).
...
- **Concurrent first-load on the same coin type is racy** — the loader writes `charts.SKASupply[coinType]` without `Lock`. Idempotent today, but a refactor must preserve or upgrade this.
...
- [ ] If the loader signature changes: update `cmd/dcrdata/internal/api/noop_ds_test.go:131` mock and any other `DataSource` implementers.

...
- New per-tx `MempoolTx` field → populate in **BOTH** `mempoolTxns` (batch) **AND** `TxHandler` (incremental); prefer one shared function — `TicketStage` demonstrates the correct approach (same `ticketStage()` function in both paths); `Fees`/`SKAFeeRates` are the canonical divergent case.
- Change `TicketStage` logic → update `ticketStage()` caller sites in **BOTH** `mempoolTxns` (`collector.go:166-169`) **AND** `TxHandler` (`monitor.go:291`); template `mempool.tmpl:270` + `ticketRowTemplate:232`; JS `mempool_controller.js::cloneTicketRow`.

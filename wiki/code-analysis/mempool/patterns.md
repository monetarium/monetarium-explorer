...
- Equivalence applies not only to aggregates but to **per-tx `MempoolTx` fields**. The two paths build `MempoolTx` from different upstream fee sources today: batch uses the node's GRM verbose result (`tx.Fee`, `SKAFeeRateMapFromAtoms`); incremental recomputes from `msgTx` (`TxFeeRate`, `SKAFeeRateMapFromVerboseVin`). Any new per-tx field MUST be populated in both `mempoolTxns` and `TxHandler`, and prefer a single shared function over re-deriving differently per path. **`TicketStage` is the reference example of the correct approach**: both paths call the same `ticketStage(vin, txnsStore)` function (`collector.go:166-169` and `monitor.go:291`). `Fees`/`SKAFeeRates` remain the canonical example of unresolved divergence.

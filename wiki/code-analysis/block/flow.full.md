...
- **Transformations:** Mirrors `explorerUI.Store()` exactly. Also hoists `GetHeightByTimestamp` + `ActiveMiners` DB queries **above** the `p.mtx.Lock()` to avoid blocking WS readers during DB round-trips. Both `CBlockSubsidy` and `ActiveMiners` are populated identically to the explorer path.
...
- **Location:** `cmd/dcrdata/public/js/controllers/mining_controller.js`, `supply_controller.js`, `home_latest_blocks_controller.js`, `blocks_controller.js`
- **Data Structures:** JSON from WS `getlatestblocksResp` / push `extra`
- **Transformations:**
  - `mining_controller.js`: reads `(ex.cblock_subsidy || ex.subsidy).pow / 1e8` for PoW reward. Renders `active_miners` count in "Mining: N miner(s)" format.
...
- **Template + WebSocket Parity (C3):** `explorerUI.Store()` and `PubSubHub.Store()` must populate `HomeInfo` identically — any field added to one must be added to the other. `CBlockSubsidy` and `ActiveMiners` are both now present in both paths; a future field addition here is a two-file change.
...
- **CBlockSubsidy in explorerUI:** `cmd/dcrdata/internal/explorer/explorer.go:574-590`
- **CBlockSubsidy in PubSubHub:** `pubsub/pubsubhub.go:741-756`
- **ActiveMiners hoisted above lock:** `pubsub/pubsubhub.go:698-703`
- **Presentation State Duplication:** `cmd/dcrdata/internal/explorer/explorer.go:~580` and `pubsub/pubsubhub.go:~760` both run identical loops transforming `map[uint8]string` → sorted `[]PoWSKAReward`

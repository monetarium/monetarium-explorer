...
- **P4 Single delegation helper consumed by both transports.** Both REST `getTicketPoolCharts` and WS `(*explorerUI).buildTicketPoolChartsData` route `mempool.{price,count,time}` through `DataSource.GetMempoolPriceCountTime` → `DataCache.GetTicketPriceCountTime` → `stakeDiff.ToCoin() + feeAvg`. The two transports cannot drift. (Pre-[#290](https://github.com/monetarium/monetarium-explorer/issues/290) the WS path reimplemented the overlay as `inv.Tickets[0].TotalOut`; do not reintroduce.) `StoreMPData` uses fail-soft semantics on bad `StakeDiff`: warns and preserves the previous value rather than zeroing it — the overlay shows last valid tick-diff, not `feeAvg` alone, on a transient bad node value.

...
- **Vitest pin** — `visualBlocks_controller.test.js` (13 tests) locks the new DOM shape: three-state vote classes, no `.block-rewards`/`.block-transactions`/`.fund` classes, no literal `DCR` text, indicator-fill TOTAL+per-coin structure, ticket coin:VAR title, and a WS-shape regression test for `normaliseWsBlock`.

Revision: `HEAD=347e5f9a` (post visualblocks UI rewrite for #270 on top of PR #284 contract; selector-based mempool-tile lookup + reconnect handler added by `793610c8`/`2218b9c`/`fe58506`/`aa356db6`; anchor→HEAD: `RemainingWindowText` extracted to `explorer/types/remaining.go`; tx-type string constants consolidated to `txhelpers.TxType*`; `HomeInfo.WindowRemaining`/`RewardRemaining` pre-computed in `Store()` but not yet read by visualblocks template).

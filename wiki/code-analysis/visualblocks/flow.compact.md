...
- **Vitest pin** — `visualBlocks_controller.test.js` (15 tests) locks the new DOM shape: three-state vote classes, no `.block-rewards`/`.block-transactions`/`.fund` classes, no literal `DCR` text, indicator-fill TOTAL+per-coin structure, ticket coin:VAR title, a WS-shape regression test for `normaliseWsBlock`, and a `reconnect` resync suite (re-request mempool on the synthetic `reconnect` event + handler cleanup on `disconnect`).
